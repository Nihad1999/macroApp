...
class MacroApp:
    def __init__(self, root):
        ...
        Kb.on_press_key('F1', self.f1_toggle_macro, suppress=True)
    ...
    def toggle_macro(self):
        ...
        if not self.is_active:
            ...
            # Update icon color if minimized
    ...
    def _execute_actions_game(self, actions):
        x, y = win32api.GetCursorPos()  # Get current mouse position
        if not self.selectedTriger_macro.get('ignore_trigger', 'no') == 'yes':
            Kb.unhook_all()
            Ms.unhook_all()
        print("Line 296--  now i am here")
...
class AddEditWindow:
    def __init__(self, parent_app, macro=None):
        ...
        if macro:
            ...
            self.game_var.set(macro.get('game_mode', 'no') == 'yes')  # Set game mode checkbox
    ...
    def save_macro(self):
        ...
        macro_data = {
            ...
            'game_mode': 'yes' if game_mode else 'no',  # Add game mode to macro data

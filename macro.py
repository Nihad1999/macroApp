...
# Then add these lines before the MacroApp class definition
elevate()
...
class MacroApp:
    def __init__(self, root):
        ...
        # Load saved macros
        self.load_macros()
        self.game_mode = False  # Added game mode initialization
    ...
    def setup_keyboard_hook(self):
        if self.is_exe():
            ...
            self.f1_handler = Kb.on_press_key('F1', self.f1_toggle_macro, suppress=True)
            # Request UI access explicitly
            ctypes.windll.user32.AllowSetForegroundWindow(ctypes.windll.kernel32.GetCurrentProcessId())
        else:
            # Regular Python script handling
            self.f1_handler = Kb.on_press_key('F1', self.f1_toggle_macro, suppress=True)


    def f1_toggle_macro(self, _):  # Added event parameter
    ...
    def toggle_macro(self):
        ...
        if not self.is_active:
            ...
            # Keep F1 suppression while adding new hooks
            current_f1 = self.f1_handler
            ...
            # Update icon color only if icon window exists
            if self.icon_window:  # Changed from hasattr check
                self.icon_window.configure(bg="green")
                self.icon_frame.configure(bg="green")
            ...
            if not self.selected_trigger_type == 'key' and not self.selected_trigger_type == 'text' and self.selectedTriger_macro.get(
                    'ignore_trigger', 'no') == 'no':
            ...
            if self.selectedTriger_macro.get('ignore_trigger',
                                             'no') == 'yes' and self.selected_trigger_type == 'key' or self.selected_trigger_type == 'text':
            ...
            # Update icon color only if icon window exists
            if self.icon_window:  # Changed from hasattr check
                self.icon_window.configure(bg="red")
    ...
    def kill_ahk_processes(self):
        try:
            ...
            print(f"Error killing AHK processes: {e}")
    ...
    def execute_macro(self):
        ...
        game_mode = self.selectedTriger_macro.get('game_mode', 'no') == 'yes'
        selected = self.macro_listbox.curselection()
        selected_index = selected[0]
    ...
    def generate_ahk_script_left(self, actions):
        script_parts = [
            "#NoEnv",
        ...
        return self._generate_common_script_parts(script_parts, actions)

    def generate_ahk_script_right(self, actions):
        script_parts = [
            "#NoEnv",
            "#SingleInstance Force",
            "SetWorkingDir %A_ScriptDir%",
    ...
    def _generate_common_script_parts(self, script_parts, actions):
        for action in actions:
            action = action.strip()
            print(f"Current action: {action}")  # This will show us each action
            if action.startswith("d:"):
                ...
                print(delay)
                script_parts.append(f"Sleep, {delay}")
            elif action.startswith("k:"):
                ...
                if click_type == "left":
                    script_parts.extend([
                        "Click",  # Using AHK's Click command instead
                        "Sleep, 25"
                    ])
                elif click_type == "right":
                    script_parts.extend([
                        "Click, right",  # Right click version
                        "Sleep, 25"
                    ])
            elif action.startswith("d:"):
                delay = int(action.split(':')[-1])
    ...
    def minimize_to_icon(self):
        ...
        bg_color = "green" if self.is_active else "red"
        ...
        # Set transparency (0.8 = 80% opacity)
        self.icon_window.attributes('-alpha', 0.5)
        self.icon_window.attributes('-topmost', 1)  # Set always on top

        self.icon_frame = tk.Frame(self.icon_window, bg=bg_color)
        self.icon_frame.pack(fill=tk.BOTH, expand=True)
        label = tk.Label(self.icon_frame, text="M", font=("Arial", 24), bg=bg_color)  # 'M' for Macro
...
class AddEditWindow:
    def __init__(self, parent_app, macro=None):
        ...
        # Add game checkbox after ignore_trigger
        # Game Mode Option
        self.game_var = tk.StringVar(value='no')
        tk.Checkbutton(self.window, text="Game Mode", variable=self.game_var,
                       onvalue='yes', offvalue='no').pack(pady=5)
        ...
        if macro:
            ...
            self.game_var.set(macro.get('game_mode', 'no'))  # Set game mode checkbox
    ...
    def save_macro(self):
        ...
        macro_data = {
            ...
            'game_mode': game_mode,  # Add game mode to macro data
